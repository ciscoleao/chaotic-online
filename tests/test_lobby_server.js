...
(async () => {
  ...
  catch (e) { ok(false, 'raiz: servidor subiu', e.message); }
  try {
    if (srvR) {
      ok(/\(lobby novo\)/.test(srvR.out()) && !/fallback/.test(srvR.out()), 'raiz: boot anuncia lobby novo');
      const gR = await req(portR, '/game', { 'x-session': SID });
      ok(gR.status === 200 && gR.body.includes('lobby_drone_side') && gR.body.includes('HeroiTeste'), 'raiz: serve lobby da raiz injetado');
    }
  } finally { if (srvR) kill(srvR.child); }
